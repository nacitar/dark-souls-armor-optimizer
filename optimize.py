...
logging.basicConfig(
    level=logging.WARNING, format="%(name)-12s: [%(levelname)-8s] %(message)s"
)
...
if False:
    ...
    best_for_modifier_gte.add(
        KnapsackItem(
            weight=KnapsackWeight(cost=0, modifier=0), value=10, name="A"
        )
    )
    print(best_for_modifier_gte.sorted())
    best_for_modifier_gte.add(
        KnapsackItem(
            weight=KnapsackWeight(cost=0, modifier=1), value=5, name="B"
        )
    )
    print(best_for_modifier_gte.sorted())
    best_for_modifier_gte.add(
        KnapsackItem(
            weight=KnapsackWeight(cost=0, modifier=2), value=3, name="C"
        )
    )
    print(best_for_modifier_gte.sorted())
    best_for_modifier_gte.add(
        KnapsackItem(
            weight=KnapsackWeight(cost=0, modifier=4), value=6, name="D"
        )
    )
    print(best_for_modifier_gte.sorted())
    best_for_modifier_gte.add(
        KnapsackItem(
            weight=KnapsackWeight(cost=4, modifier=-1), value=5, name="E"
        )
    )  # needs to become 10
    print(best_for_modifier_gte.sorted())
